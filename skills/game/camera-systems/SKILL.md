...
name: camera-systems
description: >
  Build game cameras that feel good in Godot — 2D follow with a deadzone, look-ahead, smoothing,
  and level-bounds clamping; 3D third-person orbit with collision and first-person look; plus
  multi-target framing and a shake hook. Engine-neutral techniques applied to the Godot camera
  nodes (Camera2D, Camera3D, SpringArm3D, PhantomCamera). Use when the user mentions camera
  follow, follow camera, deadzone, look-ahead, camera smoothing, camera bounds/limits,
  third-person camera, orbit camera, first-person look, or camera jitter.
---
...
`game-feel` (this skill exposes the shake offset hook it drives). For the scene structure the
camera is parented to, use `godot-nodes-scenes`. For player movement itself use the movement
script in your own project (no movement skill is curated here). For performance of many
cameras/render targets, see `performance-optimization`.

## Core workflow

1. **Decide what the camera serves.** A jump-focused game (lead the jump, see hazards), top-down
   (center with deadzone), third-person (orbit + collision), first-person (look only). The
   situation sets the rules.
2. **Follow smoothly and frame-rate independently.** Move the camera toward the target with
   exponential smoothing (`1 - exp(-rate*dt)`) or a hand-rolled spring, not a fixed
   `lerp(a, b, 0.1)` — that 0.1 is per-frame and changes with frame rate.
...
# Godot alternative: Camera2D position smoothing, or a hand-rolled critically-damped spring.
```
...
# PhantomCamera is the declarative alternative when you want a rig authored as data.
```
...
# The offset is written by game-feel; the camera only exposes and applies it.
```

## Pitfalls

- **`lerp(pos, target, const)` per frame** is frame-rate dependent — floatier at 30 FPS, snappier
  at 144. Use `1 - exp(-rate*dt)` or `Camera2D` position smoothing.
- **Following in the normal update before the target has moved** yields a one-frame lag jitter.
  Follow in `_process` *after* movement/physics resolve, or from a `Camera2D`.
...
- **Snapping on teleport/respawn** is jarring; either hard-cut intentionally (and reset
  smoothing) or fast-ease. Don't let a huge smoothing distance whip across the level.
...
- For the exponential-smoothing/spring derivation, a complete deadzone+look-ahead+bounds 2D rig,
  ...
  cinematic camera blends, and the Godot Camera2D / Camera3D / PhantomCamera mapping, read
  `references/follow-and-framing.md`.

## Related skills

- `game-feel` — owns screen-shake trauma/triggers; this skill exposes the offset it writes.
- `godot-nodes-scenes` — the scene tree the camera is parented to.
